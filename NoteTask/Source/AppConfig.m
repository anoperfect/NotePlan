...
- (NSDictionary*)configNoteUpdateDetect:(NoteModel*)note fromNoteIdentifier:(NSString*)identifier
{
   ...
   \[\@"[a-z]+"\][ ]+=
    ...
    noteDict[@"identifier"]       = note.identifier;
...
{
    NSDictionary *updateDict = [self configNoteUpdateDetect:note fromNoteIdentifier:note.identifier];
    if(updateDict.count > 0) {
...
{
    NSDictionary *updateDict = [self configNoteUpdateDetect:note fromNoteIdentifier:identifier];
    if(updateDict.count > 0) {
        [self.dbData DBDataUpdateDBName:DBNAME_CONFIG
                                toTable:TABLENAME_NOTE
                         withInfoUpdate:updateDict
                          withInfoQuery:@{@"identifier":identifier}];
    }
    else {
        NSLog(@"configNoteUpdate : nothing to update.");
    }
